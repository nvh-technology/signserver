...
namespace RSSPAPI
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            if (string.IsNullOrEmpty(PATH_TO_FILE_CONFIG) || string.IsNullOrEmpty(relyingPartyKeyStore) || string.IsNullOrEmpty(passCode) || string.IsNullOrEmpty(filePDF) || (string.IsNullOrEmpty(userID) && string.IsNullOrEmpty(credentialID)))
            {
                Console.WriteLine("Usage: Program.exe --fileConfig <path> --keystoreFile <path> (--userID <id> | --credentialID <id>) --passCode <pass> --filePDF <path> [--reason <text>] [--location <text>] [--backgroundPath <path>] [--offset <x,y>] [--boxSize <width,height>] [--titleText <text>] [--page <page>] [--position <llx,lly,urx,ury>]");
                Console.WriteLine("  --fileConfig: Path to the configuration file.");
                Console.WriteLine("  --userID: User ID for listing certificates (required if --credentialID is not used).");
                Console.WriteLine("  --credentialID: Specific credential ID to use (skips certificate listing).");
                Console.WriteLine("  --passCode: Passcode for authorization.");
            ...
            try
            {
                ...
                foreach (byte[] s in signatures)
                {
                    ...
                    foreach (byte[] result in results)
                    {
                        ...
                        Console.WriteLine($"Signed PDF saved as {fullOutputPath}");
                    }
